...
- **基线**:`lazy-novel v0.0.10`(单文件 3610 行,JS)
- **后续 spec**:Spec 2(书库/扫描/标签)、Spec 3(主题/书签/搜索)

## 1. 目标

将 lazy-novel 单文件 `extension.js` 重构为多文件模块化结构,迁移持久化数据,重命名项目为 `lazy-novel`,**完整保留现有行为**。同时为后续 Spec 2/3 铺路(添加少量基础设施)。
...
- ❌ 删除 `lazy-novel.*` 旧 globalState 键(下一版本再清空,本次仅迁移)
- ❌ 引入 TypeScript(保持 JS)
- ❌ 引入打包工具(继续 ES modules + CommonJS 混用,沿用 lazy-novel 风格)

## 3. 现有行为保留清单(Spec 1 必须 100% 等价)

| #   | 行为                                                       | 来源(lazy-novel 中位置)                         |
| --- | ---------------------------------------------------------- | ----------------------------------------------- |
...
│   │   └── Migration.js               # lazy-novel.files → lazy-novel.books(本 spec 实现)
│   ├── chapter/
...
```
if (!context.globalState.get('lazy-novel.books') && context.globalState.get('lazy-novel.files')) {
  const oldFiles = context.globalState.get('lazy-novel.files');
  ...
  context.globalState.update('lazy-novel.migrated', { from: 'lazy-novel.files', at: Date.now() });
  // 保留旧键不清空
}
```

迁移日志:`console.log('[lazy-novel] migrated N books from lazy-novel.files')`。
...
| 字段             | 旧值                                          | 新值                                                                                   |
| ---------------- | --------------------------------------------- | -------------------------------------------------------------------------------------- |
| `name`           | `"lazy-novel"`                                | `"lazy-novel"`                                                                         |
| `displayName`    | `"lazy-novel"`                                | `"Lazy Novel"`                                                                         |
| `description`    | `"一个隐蔽的 VSCode 阅读器插件..."`           | `"基于状态栏隐蔽阅读的 VSCode 插件,支持 PDF/TXT/EPUB,带书库与主题管理(详见 spec 2/3)"` |
| `publisher`      | `"neroneroffy"`                               | 保留(原作者)                                                                           |
| `repository`     | `"https://github.com/neroneroffy/lazy-novel"` | `"https://github.com/neroneroffy/lazy-novel"` + 注明 "fork for lazy-novel"             |
...
| 旧 ID                             | 新 ID                            | 默认快捷键         |
| --------------------------------- | -------------------------------- | ------------------ |
| `lazy-novel.open`                 | `lazyNovel.open`                 | (无)               |
| `lazy-novel.previousPage`         | `lazyNovel.previousPage`         | `Alt+Shift+Left`   |
| `lazy-novel.nextPage`             | `lazyNovel.nextPage`             | `Alt+Shift+Right`  |
| `lazy-novel.scrollLeft`           | `lazyNovel.scrollLeft`           | `Alt+Left`         |
| `lazy-novel.scrollRight`          | `lazyNovel.scrollRight`          | `Alt+Right`        |
| `lazy-novel.toggleVisibility`     | `lazyNovel.toggleVisibility`     | `Shift+Space`      |
| `lazy-novel.toggleChapterPreview` | `lazyNovel.toggleChapterPreview` | (无)               |
| `lazy-novel.showHoverPreview`     | `lazyNovel.showHoverPreview`     | `Ctrl+Alt+H`       |
| `lazy-novel.hideHoverPreview`     | `lazyNovel.hideHoverPreview`     | `Ctrl+Alt+Shift+H` |
...
| 旧                    | 新                    |
| --------------------- | --------------------- |
| `lazy-novel-explorer` | `lazy-novel-explorer` |
| `lazy-novel-main`     | `lazy-novel-main`     |

### 5.4 globalState 键

| 旧键                      | 新键                              |
| ------------------------- | --------------------------------- |
| `lazy-novel.files`        | `lazy-novel.books`                |
| `lazy-novel.readingState` | `lazy-novel.readingState`         |
...
| **`Migration.js` 独立模块**                 | 未来 lazy-novel → lazy-novel 迁移复用 | 极小   |
...
沿用 lazy-novel 已有的 `.vscode-test.mjs` 配置,扩展测试覆盖以下**关键路径**:

1. **激活**:扩展加载,`lazyNovel.*` 命令注册到 VSCode
2. **数据迁移**:`lazy-novel.files` 存在时,启动后 `lazy-novel.books` 写入
...
- **Spec 4(可选)**:打包发布、性能优化、清理 lazy-novel 旧键
