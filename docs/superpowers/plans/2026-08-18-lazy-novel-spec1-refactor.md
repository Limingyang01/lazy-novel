...
**Goal:** Refactor `thief-reader` v0.0.10 from a 3610-line single file `extension.js` into 16 focused modules under `src/`, rename the project to `lazy-novel`, migrate persisted state from `thief-reader.*` keys to `lazy-novel.*` keys, and preserve all existing user-visible behavior.
...
- Migration: if `lazy-novel.books` is missing and `thief-reader.files` exists, copy and translate; never delete the old key.
...
**Interfaces:**
- Consumes: existing `package.json` from thief-reader v0.0.10
...
```json
{
  ...
  "icon": "main-icon.png",
  "repository": "https://github.com/neroneroffy/thief-reader",
  "author": {
    "name": "neroneroffy",
...
- [ ] **Step 3: Verify no `thief-reader` strings remain**

Run (PowerShell):
```powershell
Select-String -Path package.json -Pattern 'thief-reader'
```
Expected: no matches.
...
Run: `npm test`
Expected: Schema suite passes; other suites (sample test from thief-reader) still pass.
...
- Produces: `migrateIfNeeded(context)` that one-shot translates `thief-reader.files` → `lazy-novel.books`; returns `{ migrated: boolean, count: number }`
...
git commit -m "feat(library): one-shot migration from thief-reader.files"
```
...
**Note**: thief-reader actually uses a fixed 80-character sliding window, not page-based pagination. But `previousPage` / `nextPage` shift by 80 chars. This task formalizes that into a `Pager` class so Spec 3 (theme) can swap implementations cleanly.
...
<head>
  ...
  <style>
    ...
    .file-list, .chapter-list { list-style: none; padding: 0; }
    .file-item, .chapter-item { padding: 4px 8px; cursor: pointer; }
    .file-item:hover, .chapter-item:hover { background: var(--vscode-list-hoverBackground); }
